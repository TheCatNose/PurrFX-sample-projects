...
#include "HackPlayer/CFrameDataProxy.h"
...
void CReversePlayerApp::showUsage()
{
	std::cout <<
		...
		"Enter [u|s|0|1|2|3] to set duty mode" << std::endl <<
...
void CReversePlayerApp::onCommand(const std::string& i_sCommand, bool& o_bQuit)
{
	if (i_sCommand == "q")
		...
		setTrack(m_nTrack+1);
	else if (i_sCommand == "u")
	{
		m_oProxy.setPulse1DutyMode(EDutyMode::Untouched);
		m_oProxy.setPulse2DutyMode(EDutyMode::Untouched);
	}
	else if (i_sCommand == "s")
	{
		m_oProxy.setPulse1DutyMode(EDutyMode::Sequence);
		m_oProxy.setPulse2DutyMode(EDutyMode::Sequence);
	}
	else if (i_sCommand.size() == 1)
	{
		char cDuty = i_sCommand[0];
		if (cDuty >= '0' &&
			cDuty <= '3' )
		{
			uint8_t nDuty = uint8_t(cDuty-'0');
			m_oProxy.setPulse1DutyValue(nDuty);
			m_oProxy.setPulse2DutyValue(nDuty);
			m_oProxy.setPulse1DutyMode(EDutyMode::Value);
			m_oProxy.setPulse2DutyMode(EDutyMode::Value);
		}
	}
}
